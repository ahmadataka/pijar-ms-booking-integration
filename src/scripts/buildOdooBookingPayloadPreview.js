...
function readArgs() {
  ...
  if (!startIso || !endIso) {
    throw new Error(
      "Usage: node src/scripts/buildOdooBookingPayloadPreview.js <startIso> <endIso>"
    );
  }
...
async function main() {
  ...
  for (const room of selectedRooms) {
    ...
    payloads.push(buildOdooBookingPayload(accessPayload));
  }
...
main().catch((error) => {
  console.error("[buildOdooBookingPayloadPreview] failed");
  console.error(error);
  process.exit(1);
});
